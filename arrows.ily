...
#(define-markup-command (Arrow layout props angle length moveX moveY)
   ...
   (interpret-markup layout props
     #{
       \markup {
         ...
         \rotate #angle
         \override #'(thickness . 1.5)    % line thickness
         \fontsize #0              % arrow-head size
         \with-dimensions #'(0 . 0) #'(0 . 0)
         \with-color #black
         \translate #(cons 0 (- 0 length))
         \combine
         \draw-line #(cons 0 length)    % can be replaced by \draw-dashed-line or \draw-dotted-line
         \translate-scaled #'(0 . -.5) % might have to be adjusted if line thickness is changed
         \with-color #black
         \override #'(layer . -10)
